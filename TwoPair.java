/* 
   Mark Porter
   Program: Poker Game Part HIGH TWO PAIR
   Assignment: DETERMINE WHETHER IT'S A HIGH TWO PAIR
   November 5, 2015
*/
...
{//opening of class TwoPair


 public int displayTwoPair(int card1, int card2, int card3, int card4, int card5, int winner)
	...
	if	((card1num == card2num) & (card3num ==card4num)  )
	{
		if (card2num == 1)
	{
	highcardname = sortsomesuits.getNumsorted(card2num, 1) ;
	System.out.printf("\n   It is a %sS HIGH TWO PAIR\n", highcardname);
	winner =  card2num + 13;
	}
		else 
	{
	highcardname = sortsomesuits.getNumsorted(card4num, 1) ;
	System.out.printf("\n   It is a %sS HIGH TWO PAIR\n", highcardname) ;
	winner =  card4num;
	}
	...
	else if ((card1num == card2num) & (card4num == card5num) )
	{
		if (card2num == 1)
	{
	highcardname = sortsomesuits.getNumsorted(card2num, 1) ;
	...
	else if ((card2num == card3num) & (card4num == card5num) )
	{
		if (card2num == 1)
	{
	highcardname = sortsomesuits.getNumsorted(card2num, 1) ;
